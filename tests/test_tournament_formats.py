...
from flaskr.core import compute_standings, fetch_entries, fetch_pairings, fetch_tournament_by_slug
...
class TournamentFormatsTestCase(unittest.TestCase):
    ...
    def create_tournament(self, name="Team Cup", *, team=True, excluded=False, capacity=""):
    ...
    def register_team(self, slug, name="Rooks", emails="alex@example.com, sam@example.com", **extra):
        return self.client.post(f"/register/{slug}", data={
            "registration_mode": "team", "team_name": name, "member_emails": emails, **extra,
        }, follow_redirects=True)
    ...
    def test_solo_assignment_preserves_answers_and_enters_team_in_pairings(self):
        ...
        self.assertIn(b"awaiting team assignment", response.data)
